...
app.post('/auth', async(req, res) => {
    ...
    const{name,pass,pin,skey,fathername,mothername,email,tel,address,gender}=req.body;
...
app.post('/check', async (req, res) => {
    ...
    try {
        ...
        console.error("Error finding user or comparing passwords:", error);
...
app.get('/viewscore', async(req, res) => {
    ...
    try {
        const username = req.session.user;
        const user = await SubmittedAnswers .findOne({ user: username });
        if (!user) {
            return res.status(404).send("User not found");
        }
        file='/images/'+user.user+'.jpg';
            res.render('score.pug',{image:file,data1:user}); 
    } 
    catch (error) {
        console.error("Error fetching user profile:", error);
        res.status(500).send("Internal Server Error");
